...
export function beginConversationGeneration(
  ...
  return {
    generatingByConversation: {
      ...state.generatingByConversation,
...
export function pruneRemovedGenerationIndexesByRunId(runIds: string[], state: GenerationState): GenerationState {
  const allowed = new Set(runIds)
  return {
    ...state,
    removedIndexesByRunId: Object.fromEntries(
