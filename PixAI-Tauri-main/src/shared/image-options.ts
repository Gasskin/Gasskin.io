...
export function buildImageEndpoint(baseUrl: string): string {
  return `${trimBaseUrl(baseUrl)}/v1/images/generations`
}

export function buildImageEditEndpoint(baseUrl: string): string {
  return `${trimBaseUrl(baseUrl)}/v1/images/edits`
}
...
export function trimBaseUrl(baseUrl: string): string {
  return (baseUrl || 'https://api.openai.com').trim().replace(/\/+$/, '')
}
...
export function normalizeImageGenerationTimeoutSeconds(value: unknown): number {
  ...
  return Math.min(1800, Math.max(1, Math.trunc(value)))
}
