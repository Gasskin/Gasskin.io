...
- stock.txt: 每行一个代码（可选买入价）；用代码前一行或行尾的「# 名称」设置展示名
- data/:     输出目录
"""
...
def read_code_entries(stock_file: Path) -> list[tuple[str, float | None, str | None]]:
    """解析 stock.txt，支持买入价以及代码前一行/行尾的 ``# 自定义名称``。"""
    ...
    for line_no, line in enumerate(stock_file.read_text(encoding="utf-8-sig").splitlines(), start=1):
        ...
        if text.startswith("#"):
            pending_name = text[1:].strip() or None
            continue

        code_text, separator, inline_comment = text.partition("#")
        ...
        custom_name = inline_comment.strip() if separator and inline_comment.strip() else pending_name
        pending_name = None
        ...
        if code in seen:
            ...
            continue
        seen.add(code)
        entries.append((code, buy_price, custom_name))
...
def main() -> int:
    ...
    for code, buy_price, custom_name in entries:
        ...
        payload = {
            "code": code,
            "name": custom_name or name_map.get(code, ""),
            "type": resolved_type,
            "buy_price": buy_price,
            "source": fetch.source,
